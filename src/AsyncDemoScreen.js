...
const CreateMScreen = () => {
    ...
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.touch} onPress={()=>{setStringValue(k, v)}}>
        ...
        <Text>{result2}</Text>
        <TouchableOpacity style={styles.touch} onPress={()=>{getAll()}}>
            <Text>Get all</Text>
        </TouchableOpacity>
        {result?.map((item, index) => {
          return (<Text key={index}>Key {index}: {item}</Text>)
        })}
        <TextInput placeholder='Remove' value={remove} onChangeText={setRemove} />
        <TouchableOpacity style={styles.touch} onPress={() => {handleRemove()}}>
          <Text>Remove One</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.touch} onPress={()=> {handleClear()}}>
          <Text>Clear all</Text>
        </TouchableOpacity>
